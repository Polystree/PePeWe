<?php
$stmt = $connect->prepare("SELECT d.*, p.name as product_name, p.image_path FROM discounts d JOIN products p ON d.product_id = p.productId ORDER BY d.start_date DESC");
$stmt->execute();
            ...
            <?php foreach ($discounts as $discount): ?>
                <tr>
                    ...
                    <td>
                        ...
                        if ($now < $start) echo '<span class="status-badge scheduled">Scheduled</span>';
                        elseif ($now > $end) echo '<span class="status-badge expired">Expired</span>';
                        else echo '<span class="status-badge active">Active</span>';
                        ?>
                    </td>
