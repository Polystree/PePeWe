...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        $stmt = $connect->prepare("INSERT INTO cart (userId, productId, product_name, price, quantity, image_path) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("iissis", $userId, $productId, $product['product_name'], $product['price'], $quantity, $product['image_path']);
    }
